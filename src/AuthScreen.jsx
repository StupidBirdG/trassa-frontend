...
const [botLink,setBotLink]=useState("https://t.me/Abon9_bot");
const [loading,setLoading]=useState(false);
const [error,setError]=useState("");
const [countdown,setCountdown]=useState(0);
const err=(msg)=>{setError(msg);setLoading(false);};
function startCountdown(){setCountdown(60);const t=setInterval(()=>setCountdown(c=>{if(c<=1){clearInterval(t);return 0;}return c-1;}),1000);}
async function handleSendCode(){
setError("");if(!phone.trim())return err("Введите номер телефона");
setLoading(true);
try{const res=await auth.sendCode(phone.trim());setSavedPhone(phone.trim());if(res.channel==="need_telegram"){if(res.botLink)setBotLink(res.botLink);setStep("telegram");setLoading(false);return;}setStep("code");setLoading(false);startCountdown();}
catch(e){err(e.message);}
}
async function handleVerify(){
setError("");if(code.length<4)return err("Введите код");setLoading(true);
...
step==="telegram"&&React.createElement(React.Fragment,null,
React.createElement("div",{style:{fontSize:36,textAlign:"center"}},"📲"),
React.createElement("div",{style:{fontSize:17,fontWeight:700,color:C.text,textAlign:"center"}},"Подтвердите номер в Telegram"),
React.createElement("div",{style:{fontSize:13,color:C.muted,lineHeight:1.5}},"Чтобы получать коды входа, привяжите номер через нашего бота:"),
React.createElement("div",{style:{fontSize:13,color:C.muted,lineHeight:1.7}},
React.createElement("div",null,"1. Откройте бота кнопкой ниже"),
React.createElement("div",null,"2. Нажмите «Старт» в Telegram"),
React.createElement("div",null,"3. Нажмите «📱 Отправить мой номер»"),
React.createElement("div",null,"4. Вернитесь сюда и нажмите «Я привязал номер»")),
React.createElement("a",{href:botLink,target:"_blank",rel:"noopener noreferrer",style:{display:"block",textAlign:"center",width:"100%",padding:"14px",borderRadius:8,background:"#229ED9",color:"#fff",fontWeight:700,fontSize:15,textDecoration:"none",boxSizing:"border-box",fontFamily:"Arial,sans-serif"}},"Открыть Telegram-бота"),
error&&React.createElement("div",{style:{fontSize:12,color:C.rust}},error),
React.createElement(Btn,{onClick:handleSendCode},"Я привязал номер — получить код"),
React.createElement(Btn,{secondary:true,onClick:()=>{setStep("phone");setError("");}},"Назад")),
...
React.createElement("div",{style:{fontSize:12,color:C.mutedDark}},"Проверьте Telegram"),
