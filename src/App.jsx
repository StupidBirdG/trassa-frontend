...
import { Truck, Package, Calendar, Star, Plus, X, CheckCircle2, Filter, Phone, Lock, Crown } from 'lucide-react';
...
function CargoCard({ cargo, role, onAcceptBid, onMarkDelivered, onBid, onNeedSub, myUserId }) {
            ...
            <div key={b.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: C.surfaceAlt, borderRadius: 7, padding: '7px 10px', gap: 8 }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                ...
                <span style={{ fontSize: 11, color: C.muted, display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 4 }}>
                  {b.truck_type} <Star size={10} fill={C.amber} color={C.amber} style={{ marginLeft: 4 }} />{b.rating ?? '—'}
          ...
          {loading ? (
            ...
            <div className="cargo-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(320px,1fr))', gap: 14 }}>
              {visibleCargos.map((cargo) => (
                <CargoCard key={cargo.id} cargo={cargo} role={role} myUserId={myUserId} onAcceptBid={acceptBid} onMarkDelivered={markDelivered} onBid={onBid} onNeedSub={() => setView('pricing')} />
              ))}
            </div>
          )}
        </>
