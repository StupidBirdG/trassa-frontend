...
{b.truck_type}
<RatingBadge userId={b.carrier_id} />
...
{!reviewed && (
<button onClick={() => { onReview(acceptedBid.id, 'shipper'); setReviewed(true); }} style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 11, fontWeight: 700, color: C.amber, background: C.amberDim, border: '1px solid ' + C.amber + '55', borderRadius: 5, padding: '6px 10px', cursor: 'pointer' }}>
<MessageSquare size={12} /> Оставить отзыв
</button>
)}
{reviewed && <span style={{ fontSize: 11, color: C.success }}>✓ Отзыв отправлен</span>}
</div>
)}
...
{!reviewed && (
<button onClick={() => { onReview(acceptedBid.id, 'carrier'); setReviewed(true); }} style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 11, fontWeight: 700, color: C.amber, background: C.amberDim, border: '1px solid ' + C.amber + '55', borderRadius: 5, padding: '6px 10px', cursor: 'pointer' }}>
<MessageSquare size={12} /> Оставить отзыв
</button>
)}
{reviewed && <span style={{ fontSize: 11, color: C.success }}>✓ Отзыв отправлен</span>}
...
<CargoCard key={cargo.id} cargo={cargo} role={role} myUserId={myUserId}
onAcceptBid={acceptBid} onMarkDelivered={markDelivered} onBid={onBid}
onNeedSub={() => setView('pricing')}
onReview={(orderId, reviewRole) => setReviewModal({ orderId, role: reviewRole })} />
))}
</div>
)}
</>
